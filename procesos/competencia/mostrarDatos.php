...
foreach ($datos as $key => $value) {
	$datosTabla = $datosTabla . '<tr>
                ...
                <td>'.$obj->nombreCarrera($value['id_competencia']).'</td>
                <td>'.$value['nombre']. '</td>
                <td>'.$value['campo_desar_asig'].'</td>
                <td>'.$value['campo_desar_proyint'].'</td>
                <td class="text-center">
                <a href="competenciaEntregable.php?id='.$value['id_competencia'].'" class="btn btn-info btn-sm fas fa-plus"></a>
                </td>
                <td class="text-center">

                    <!-- botones mandan por medio de onclick a mis funciones en mysite.js -->
                    <!-- Editar -->
                    <span class="btn btn-info btn-sm" data-toggle="modal" data-target="#editarRegistro" data-backdrop="static" id="editar" onclick="obtenerDatos('.$value['id_competencia'].')">
                        <span class="fas fa-edit"></span>
                    </span>

                    <!-- Eliminar -->
                    <span class="btn btn-danger btn-sm ml-2" id="eliminar" onclick="eliminarDatos('.$value['id_competencia'].')">
